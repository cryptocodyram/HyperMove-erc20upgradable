...
describe("HyperBurnUpgradeable Token", () => {
  ...
  describe("hBurn - mint and burn", () => {
    it("#mint - 1 Billion token", async () => {
      await expect(
        hBurn.connect(owner).mint(owner.address, parse("1000000000"))
      )
        .to.be.emit(hBurn, "Transfer")
        .withArgs(ZERO_ADDRESS, owner.address, parse("1000000000"));
    });
    it("#burn - 500 million token", async () => {
      await expect(hBurn.connect(owner).burn(owner.address, parse("500000000")))
        .to.be.emit(hBurn, "Transfer")
    ...
    it("#balanceOf", async () => {
      expect(await hBurn.balanceOf(address1.address)).to.be.equals(
        parse("100000000")
      );
    });
  });
  ...
  describe("HBurn Ownable", () => {
    ...
    it("#transferOwnership", async () => {
      await expect(hBurn.connect(owner).transferOwnership(address1.address))
